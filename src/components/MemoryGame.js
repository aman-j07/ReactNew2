...
const MemoryGame = () => {
  ...
  return (
    <>
      <h2>Memory Game</h2>
      <div ref={refDiv} id="congDiv" className="hidden"><h2>Congratulations !!!</h2><p>You Won</p><p>You took {steps} steps.</p></div>
      <div className="container">
        {values.map((item,i)=>{
          return(<FlipCard key={i} ind={item[0]}  onclick={show} img={item[1]} />)
        })}
      </div>
    </>
  );
